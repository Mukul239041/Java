...
public class TestBanking {
	...
	public static void main(String[] args) {
		try (Scanner sc = new Scanner(System.in)) {
			...
			accountMap=populateAccountMap();
			boolean exit = false;
			while (!exit) {
				System.out.println("Options 1. Display all accts 2. Add Bank Account 3. Disable Account"
						+ " 4. Deposite Amount 5. Withdraw Amount 6. Transfer Amount "
						+ "7. Sort As Per AccNo 8. Sort by Date And Name 9.Sign Up"
						+ " 10.Sign In 11.ChangePassword 12.Sorting as per Natural order. 13. Exit");
				try {
					switch (sc.nextInt()) {
					case 1:
					...
					case 12:
						...
						accountMap.values().stream().sorted().forEach(System.out::println);//Natural Sorting
						break;
